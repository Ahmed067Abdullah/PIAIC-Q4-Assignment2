...
const PetCard = ({ pet, onAdopt, onUnadopt, adopted, userAddress }) => {
  return <div key={pet.id} style={{ border: "1px solid black", display: "inline-block", padding: "20px", margin: "10px" }}  >
    ...
    <div>
      ...
      {adopted === "0x0000000000000000000000000000000000000000"
        ? <button type="button" onClick={() => onAdopt(pet.id)}>Adopt</button>
        : adopted === userAddress
          ? <button type="button" onClick={() => onUnadopt(pet.id)}>Unadopt</button>
